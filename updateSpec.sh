...
for v in `ls ~/.cocoapods/repos/HETSDKSpecs/`; do
	if [[ $v ==  ${podSpecDir} ]]; then
		haveThisVersion="YES"
	fi
done
if [[ ${haveThisVersion} == "NO" ]]; then
	mkdir ~/.cocoapods/repos/HETSDKSpecs/${podSpecDir}
fi

haveThisVersion="NO"
for v in `ls ~/.cocoapods/repos/HETSDKSpecs/${podSpecDir}`; do
	if [[ ${v} == ${version} ]]; then
		haveThisVersion="YES"
	fi
done
...
if [[ ${haveThisVersion} == "NO" ]]; then
	mkdir ~/.cocoapods/repos/HETSDKSpecs/${podSpecDir}/${version}
fi
cp ${podName} ~/.cocoapods/repos/HETSDKSpecs/${podSpecDir}/${version}/${podName}

#append commit id to .podspec
commitId=`git log --pretty=oneline | sed -n '1p' | cut -d " " -f 1`
preDir=`pwd`
cd  ~/.cocoapods/repos/HETSDKSpecs/
